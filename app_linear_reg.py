...
def load_css(file):
    with open(file) as f:
        st.markdown(f"<style>{f.read()}</style>", unsafe_allow_html=True) #allowing the html file also since it donot allow html file by default
...
st.markdown("""
            <div class = "card">
                ...
                <p> Predict <b Tip Amount </b> from <b> Total Bill </b> using Linear Regression Model </p>
            </div>
...
c3,c4 = st.columns(2)
c3.metric("R-squared (R2)", f"{r2:.3f}")
